...
function ProjectDetails({ params }) {
  ...
  return (
    <section className='relative'>
      <div className='group absolute bg-white w-10 h-10 sm:w-14 sm:h-14 flex justify-center items-center font-black xl:text-2xl p-6 sm:p-8 rounded-full top-5 left-5 sm:top-10 sm:left-10 borderBlack transition-all hover:scale-110 dark:bg-[#c79e55] dark:border-4 dark:border-[#242426] cursor-pointer'>
      ...
      <div className='w-[80%] mx-auto font-medium pt-28 sm:pt-40 xl:pt-52 xl:px-20 flex flex-row justify-center'>
        <div className='w-[100%] xl:w-[70%] xl:pr-10 flex flex-col'>
          <h1 className='text-4xl sm:text-6xl xl:text-4xl font-bold mb-2 break-words' style={{display:'inline-block', color:'transparent', WebkitBackgroundClip:'text', backgroundImage:theme === "light"?'linear-gradient(to bottom right, #dac24a, #ac8c31':'linear-gradient(to bottom right, #c79e55, #c79e55'}}>{selectedProject.name}</h1>
          ...
          <ul className='flex flex-row gap-2 justify-center flex-wrap mb-8 xl:hidden'>{selectedProject.tags && selectedProject.tags.map((tag, index) => (
            <li key={index} className='bg-white/70 border border-[#dac24a]/50 rounded-xl px-5 py-3 dark:bg-white/10 dark:text-white/80 dark:border-[#c79e55]/50'>{tag}</li>
          ))}
          </ul>
          ...
          <p className='text-[#ac8c31] font-bold mt-8 flex flex-row items-center gap-1 -mb-3 z-40 dark:text-[#c79e55]'>Notes <PiNotepadFill className='text-lg'/></p>
          ...
          <div className='flex flex-row gap-4 flex-wrap my-16 xl:my-10'>
            <a href={selectedProject.liveUrl? selectedProject.liveUrl : selectedProject.GithubUrl} className={clsx('bg-gradient-to-br from-[#dac24a] to-[#ac8c31] text-white px-7 py-3 flex items-center gap-2 rounded-full outline-none focus:scale-110 hover:scale-110 hover:bg-[#f2db6a] active:scale-105 transition dark:from-[#c79e55] dark:to-[#c79e55]',{"opacity-50 pointer-events-none" : selectedProject.liveUrl===''})} target='_blank'>live link <FaExternalLinkAlt /></a>
            <a href={selectedProject.GithubUrl} className='bg-gradient-to-br from-[#dac24a] to-[#ac8c31] text-white px-7 py-3 flex items-center gap-2 rounded-full outline-none focus:scale-110 hover:scale-110 hover:bg-[#f2db6a] active:scale-105 transition dark:from-[#c79e55] dark:to-[#c79e55]' target='_blank'>GitHub <FaGithubSquare className='text-lg'/></a>
            <a href={selectedProject.videoUrl? selectedProject.videoUrl : selectedProject.GithubUrl} className={clsx('border-[0.2rem] border-[#ac8c31] px-7 py-3 flex items-center gap-2 rounded-full outline-none focus:scale-110 hover:scale-110 active:scale-105 transition dark:border-[#c79e55]',{"opacity-50 pointer-events-none" : selectedProject.videoUrl===''})} target='_blank'>watch demo <FaCirclePlay className='text-lg'/></a>
          </div>
        </div>
