    ...
    return (
        <motion.div className='last:hidden sm:last:block rounded-lg relative group border border-black/40' variants={eachProjectAnimationVariants} initial="initial" whileInView="animate" viewport={{once: true}} custom={index}>
            ...
            <div className='hidden group-hover:flex absolute gap-9 -translate-x-[50%] -translate-y-[50%] top-[50%] left-[50%]'>
                <Link href={project.url} className='text-[#ac8c31] h-16 w-16 rounded-full bg-white bg-opacity-70 hover:text-[#dac24a] hover:bg-opacity-100 text-3xl flex justify-center items-center transition-all dark:text-[#ae7c26] dark:hover:text-[#c79e55]' target='_blank'><FaExternalLinkAlt /></Link>
                <Link href={`/allprojects/${project.name}`} className='text-[#ac8c31] h-16 w-16 rounded-full bg-white bg-opacity-70 hover:text-[#dac24a] hover:bg-opacity-100 text-4xl flex justify-center items-center dark:text-[#ae7c26] dark:hover:text-[#c79e55]'><IoMdInformationCircle /></Link>
            </div>
        </motion.div>
    )
}
