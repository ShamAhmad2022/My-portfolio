...
function Intro() {
  ...
  return (
    <section ref={ref} id='home' className='mb-28 max-w-[50rem] text-center sm:mb-0 scroll-mt-[50rem]'>
        ...
        <motion.h1 className='mb-10 mt-4 px-4 text-2xl font-medium !leading-[1.5] sm:text-4xl' initial={{opacity:0, scale:0}} animate={{opacity:1, scale:1}}>
            <span className='font-bold'>Hello, I'm <TypeAnimation
            sequence={[
              "Sham Al-Jalam", 1000
            ]}
            speed={10}
        ...
        <motion.div className='flex flex-col sm:flex-row items-center justify-center gap-2 px-4 text-lg font-medium' initial={{opacity: 0, y: 100}} animate={{opacity:1, y: 0}} transition={{delay: 0.1}}>
            ...
            <Link href="#contact" className='group bg-pink-900 text-white px-7 py-3 flex items-center gap-2 rounded-full outline-none focus:scale-110 hover:scale-110 hover:bg-pink-950 active:scale-105 transition dark:hover:bg-pink-900' onClick={()=>{setActiveSection("Contact"); setTimeOfLAstClick(Date.now());}}>Contact me <BsArrowRight className='opacity-70 group-hover:translate-x-1.5 transition'/> </Link>
            
            <a className='group bg-white px-7 py-3 flex items-center gap-2 rounded-full outline-none focus:scale-110 hover:scale-110 active:scale-105 transition cursor-pointer borderBlack dark:bg-white/10' href="/Sham-Al-Jalam.pdf" download>Download CV <HiDownload className='opacity-60 group-hover:translate-y-1 transition'/> </a>

            <a href='https://www.linkedin.com/in/sham-al-jalam' target='_blank' className='bg-white text-pink-900 p-4 flex items-center gap-2 rounded-full focus:scale-[1.15] hover:scale-[1.15] hover:text-pink-950 active:scale-105 transition cursor-pointer borderBlack dark:bg-white/10 dark:text-white/60'>
              <BsLinkedin />  
            </a>

            <a href='https://github.com/ShamAhmad2022' target='_blank' className='bg-white text-pink-900 p-4 flex items-center gap-2 rounded-full focus:scale-[1.15] hover:scale-[1.15] hover:text-pink-950 active:scale-105 transition cursor-pointer borderBlack dark:bg-white/10 dark:text-white/60'>
              <FaGithubSquare />  
            </a>
        </motion.div>
    </section>
